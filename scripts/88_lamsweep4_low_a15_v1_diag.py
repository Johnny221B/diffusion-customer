"""88 - lambda={0.5,1,10,50} sweep diagnostic, v=1.0, alpha=15, competitor low/36 (D_B=0.5394, 15pct).

Same 4 panels as 82/84 but SIX lambdas, competitor low/36 (D_B=0.5394, 15th pct -> win-rate
...
Usage:  conda run -n diverse --no-capture-output python scripts/88_lamsweep4_low_a15_v1_diag.py [STAMP]
"""
...
D_B = 0.5393660069            # competitor (low,36); y=1 iff ds_to_R < D_B
...
if len(sys.argv) > 1:
    ...
    cands = sorted(glob.glob("outputs/cmts_lam50_v1.0_blow_a15_d16_B8_T200_*"))
    if not cands:
        sys.exit("no cmts_lam*_v1.0_blow_a15_* runs found; pass STAMP explicitly")
...
def load_cfg(lam):
    d = f"outputs/cmts_lam{lam}_v1.0_blow_a15_d16_B8_T200_{STAMP}"
...
axBEST.annotate(f"$D_B$={D_B:.4f} (low/36)", (0, D_B), fontsize=8, color="gray", va="bottom")
...
fig.suptitle(f"CM-TS $\\lambda$-sweep {{0.5..50}}, v=1.0, B=low/36 "
             f"($D_B$=0.5394, 15pct; $\\alpha$=15, d=16, T=200, B=8, {STAMP})", fontsize=12)
fig.tight_layout(rect=[0, 0, 1, 0.96])
png = os.path.join(OUT, f"lamsweep4_low_a15_v1_diag_{STAMP}.png")
...
ax.set_title(f"per-round $\\|\\hat\\beta_t\\|$ — v=1.0, B=low/36, $\\alpha$=15 ({STAMP})\n"
...
fig2.tight_layout()
png2 = os.path.join(OUT, f"beta_norm_lamsweep4_low_{STAMP}.png")
