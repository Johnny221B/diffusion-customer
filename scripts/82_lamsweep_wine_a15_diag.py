"""82 - lambda={50,5} sweep diagnostic, v=4, alpha=15, competitor wine/34 (rising-trend band).

Same 4 panels as 81 but competitor = wine/34 (D_B=0.5198, 10th pct) -- the predicted sweet
spot between black/18 (too strong, caps 0.75) and pigment/12 (too weak, pinned 1.0).
Validates the prediction: does win-rate climb ~0.3 -> ~0.85 (not pinned), and does best-so-far
ds drop back toward black's 0.2803?

Usage:  conda run -n diverse --no-capture-output python scripts/82_lamsweep_wine_a15_diag.py [STAMP]
"""
...
if len(sys.argv) > 1:
    ...
    cands = sorted(glob.glob("outputs/cmts_lam50_v4.0_bwine_a15_d16_B8_T200_*"))
    if not cands:
        sys.exit("no cmts_lam50_v4.0_bwine_a15_* runs found; pass STAMP explicitly")
...
def load_cfg(lam):
    d = f"outputs/cmts_{lam}_v4.0_bwine_a15_d16_B8_T200_{STAMP}"
...
fig.suptitle(f"CM-TS $\\lambda$={{50,5}} sweep, v=4, B=wine/34 — rising-trend band "
             f"($D_B$=0.5198, 10pct; $\\alpha$=15, d=16, T=200, B=8, {STAMP})", fontsize=12)
fig.tight_layout(rect=[0, 0, 1, 0.96])
png = os.path.join(OUT, f"lamsweep_wine_a15_diag_{STAMP}.png")
