#!/usr/bin/env bash
# Resume the five completed alpha=30, v=2, lambda=50 trajectories at t=200
# and extend them through t=299. Checkpoints and final outputs stay in the
# canonical 0621_0437 directory; completed rounds are not rendered again.
set -u
...
launch () {  # gpu seed_start seed_end partial_id
  ...
  env -u LD_LIBRARY_PATH CUDA_VISIBLE_DEVICES=$G \
    ...
    python scripts/73_cmts_dreamsim.py \
      ...
      --dim 16 --T 300 --B 8 --n0 24 \
      --v 2.0 --S 8.0 --lam 50 --alpha 30 \
      --save_img_every 10 --partial_id "$PID" --tag cmts \
      --out_root "$OUT" \
      >> "$OUT/extend_T300_g${G}.log" 2>&1 </dev/null &
  echo "GPU$G sims=[$SS,$SE) pid=$!"
}

echo "=== extend a30_v2_lam50: T200 -> T300, five paired trajectories ==="
launch 0 0 2 10
launch 1 2 3 11
launch 2 3 4 12
launch 3 4 5 13
