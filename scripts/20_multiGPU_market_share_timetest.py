...
def worker_fn(rank, task_queue, result_queue, model_path, ref_image_path, dist_competitor):
    ...
    while True:
        ...
        t_encgen = float(time.time() - t_encgen0)
        ...
        for i, img in enumerate(images):
            ...
            img.close()
        t_score = float(time.time() - t_score0)

        result_queue.put((idx_chunk, y_list, d_list, t_encgen, t_score))
...
def main():
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        t_sample = float(time.time() - t_sample0)
        ...
        for _ in range(world_size):
            idx_chunk, y_list, d_list, t_encgen, t_score = result_queue.get()  # 先不加 timeout 也行
            y_buf[idx_chunk] = np.asarray(y_list, dtype=np.int64)
            d_buf[idx_chunk] = np.asarray(d_list, dtype=np.float32)
            encgen_times.append(float(t_encgen))
            score_times.append(float(t_score))
            
        t_gather = float(time.time() - t_gather0)
        
        encgen_max = float(np.max(encgen_times))
        encgen_mean = float(np.mean(encgen_times))
        score_max = float(np.max(score_times))
        score_mean = float(np.mean(score_times))
        ...
        opt.update_posterior()
        t_update = float(time.time() - t_update0)
        
        current_share = float(y_buf.mean())
        current_dist  = float(d_buf.mean())
        regret = float(oracle_share - current_share) 
        
        t_epoch = float(time.time() - t_epoch0)

        print(
            f"[Epoch {epoch:05d}] total={t_epoch:6.1f}s | "
            f"sample_z={t_sample:5.1f}s "
            f"gather={t_gather:5.1f}s | worker(enc+gen) mean/max={encgen_mean:5.1f}/{encgen_max:5.1f}s | "
            f"worker(score) mean/max={score_mean:5.1f}/{score_max:5.1f}s | "
            f"update={t_update:5.1f}s"
        )
