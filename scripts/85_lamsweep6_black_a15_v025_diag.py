"""85 - lambda={0.5,1,5,10,20,50} sweep diagnostic, v=0.25, alpha=15, competitor black/18.

Same 4 panels as 82/84 but SIX lambdas, strong competitor black/18 (D_B=0.4704, 3rd pct -> win-rate
headroom), and v=0.25 (greedy regime: small draw cov, CM-TS -> argmax on beta_hat's direction).
...
Usage:  conda run -n diverse --no-capture-output python scripts/85_lamsweep6_black_a15_v025_diag.py [STAMP]
"""
...
D_B = 0.4704190493            # competitor (black,18); y=1 iff ds_to_R < D_B
...
if len(sys.argv) > 1:
    ...
    cands = sorted(glob.glob("outputs/cmts_lam50_v0.25_bblack_a15_d16_B8_T200_*"))
    if not cands:
        sys.exit("no cmts_lam*_v0.25_bblack_a15_* runs found; pass STAMP explicitly")
    STAMP = re.search(r'_T200_(\d{4}_\d{4})$', cands[-1]).group(1)
print(f"STAMP = {STAMP}")

# lambda -> (label, color); ordered small..large
CMAP = plt.get_cmap("viridis")
LAMS = ["0.5", "1", "5", "10", "20", "50"]
COLORS = {lam: CMAP(i / (len(LAMS) - 1)) for i, lam in enumerate(LAMS)}
LABELS = {"0.5": "$\\lambda$=0.5", "1": "$\\lambda$=1", "5": "$\\lambda$=5",
          "10": "$\\lambda$=10", "20": "$\\lambda$=20", "50": "$\\lambda$=50"}


def load_cfg(lam):
    d = f"outputs/cmts_lam{lam}_v0.25_bblack_a15_d16_B8_T200_{STAMP}"
...
axBEST.annotate(f"$D_B$={D_B:.4f} (black/18)", (0, D_B), fontsize=8, color="gray", va="bottom")
...
fig.suptitle(f"CM-TS $\\lambda$-sweep {{0.5..50}}, v=0.25 (greedy), B=black/18 strong "
             f"($D_B$=0.4704, 3pct; $\\alpha$=15, d=16, T=200, B=8, {STAMP})", fontsize=12)
fig.tight_layout(rect=[0, 0, 1, 0.96])
png = os.path.join(OUT, f"lamsweep6_black_a15_v025_diag_{STAMP}.png")
...
ax.set_title(f"per-round $\\|\\hat\\beta_t\\|$ — v=0.25, B=black/18, $\\alpha$=15 ({STAMP})\n"
...
fig2.tight_layout()
png2 = os.path.join(OUT, f"beta_norm_lamsweep6_black_{STAMP}.png")
