"""86 - lambda={0.5,1,5,10,50} sweep diagnostic, v=1.0, alpha=15, competitor black/18.
...
Usage:  conda run -n diverse --no-capture-output python scripts/86_lamsweep5_black_a15_v1_diag.py [STAMP]
"""
...
if len(sys.argv) > 1:
    ...
    cands = sorted(glob.glob("outputs/cmts_lam50_v1.0_bblack_a15_d16_B8_T200_*"))
    if not cands:
        sys.exit("no cmts_lam*_v1.0_bblack_a15_* runs found; pass STAMP explicitly")
...
def load_cfg(lam):
    d = f"outputs/cmts_lam{lam}_v1.0_bblack_a15_d16_B8_T200_{STAMP}"
...
fig.suptitle(f"CM-TS $\\lambda$-sweep {{0.5..50}}, v=1.0, B=black/18 strong "
             f"($D_B$=0.4704, 3pct; $\\alpha$=15, d=16, T=200, B=8, {STAMP})", fontsize=12)
fig.tight_layout(rect=[0, 0, 1, 0.96])
png = os.path.join(OUT, f"lamsweep5_black_a15_v1_diag_{STAMP}.png")
...
ax.set_title(f"per-round $\\|\\hat\\beta_t\\|$ — v=1.0, B=black/18, $\\alpha$=15 ({STAMP})\n"
...
fig2.tight_layout()
png2 = os.path.join(OUT, f"beta_norm_lamsweep5_black_{STAMP}.png")
