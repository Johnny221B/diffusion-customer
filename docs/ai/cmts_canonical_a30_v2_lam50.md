...
> 当前状态：同一批 5 条 trajectory 已配对续跑到 T=1000。
...
- 配对续跑至 T=1000 后，最后 20 轮 true-p 达到 `0.6988 ± 0.0604 SE`，
  hard win-rate 达到 `0.9588`，mean distance 降至 `0.4380`。
...
| `T` | 1000 completed | BO rounds per trajectory |
...
同一批 5 条 trajectory 已从 checkpoint `t=500` 续跑到 `t=1000`：

```bash
bash scripts/99_extend_a30_v2_lam50_T1000.sh
```

这仍是同一组 paired continuation，不是新实验。全部 `sim000`–`sim004` 的最大 `t` 都是 999。

| 窗口 | hard win-rate | true-p | predicted-p | mean ds |
|---|---:|---:|---:|---:|
| rounds 480–499 | 0.9213 | 0.6528 | 0.6551 | 0.44705 |
| rounds 900–949 | 0.9500 | 0.6942 | 0.6860 | 0.43899 |
| rounds 950–999 | 0.9545 | 0.6957 | 0.6882 | 0.43866 |
| rounds 980–999 | 0.9588 | **0.6988** | 0.6889 | 0.43804 |
| rounds 990–999 | 0.9575 | 0.6961 | 0.6889 | 0.43840 |

T=500→1000 仍主要改善生成分布中心：mean ds 从 `0.44705` 降到 `0.43804`，true-p 从
`0.6528` 升到 `0.6988`。best-so-far ds 只从 `0.3613` 小幅到 `0.3544`，说明后 500 轮不是靠偶然
发现单张极低距离图，而是让每轮 batch 的平均质量继续提高。后段 predicted-p 略低于 true-p
（last20 gap ≈ `-0.0099`），没有出现 saturation；T=999 平均 `||β||≈0.057`，远低于 `S=8`。

T=1000 主图：`outputs/cmts_top4_summary_0621_0437/a30_v2_lam50_T1000_diagnostics_with_ceilings.png`

T=1000 beta/cov 诊断：`outputs/cmts_top4_summary_0621_0437/a30_v2_lam50_theta_eigs_T1000.png`
...
- T=500 diagnostic：`scripts/98_a30_v2_lam50_T500_diag.py`
- T=1000 diagnostic：`scripts/101_a30_v2_lam50_T1000_diag_with_ceilings.py`
- T=1000 beta/cov diagnostic：`scripts/100_a30_v2_lam50_theta_eigs_diag.py`
...
目录名保留首次 T=200 launch 的名字，但内部 trajectory 已续到 T=1000。
...
- T=1000 后分布中心是否仍值得继续优化，以及边际 GPU 成本是否合理。
